...
int cadastrar_cliente(Cliente * c) {
    ...
    if(check_cli_name(nome) == 1) return 1;
    strupr(nome);
    strcpy(c->nome, nome);
    c->cadastro = 1;
...
// retorna 0 se o nome é valido e retorna 1 caso o processo seja cancelado;
int check_cli_name(char * s) {
    ...
    while(1) {
        ...
        strupr(s); // transforma todas as letras para maiusculo;
        ...
        while(1){
            ...
            if(s[i] == '\0') break;
            if((!(s[i] >= 'A' && s[i] <= 'Z')) && s[i] != ' ') aux++; // Se tem alguma letra diferente de A-Z e ' '
            i++;
        }
        
        if(aux > 0)printf("Nome de cliente inválido, apenas é permitidos letras de A-Z e espaços, sem acentos ou caracteres especiais, digite um nome válido.\n");
...
int pick_date(Data * d) {
    ...
    while(1) {
        ...
        if(num == 0) return 1; // cancelou o processo ;
            ...
            printf("Dia de data inválido, digite um dia válido.\n");
        }
        else {
    ...
    while(1) {
        ...
        else if (num == 2 && d->dia > 29)printf("Mês de nascimento inválido, não possui o dia digitado em fevereiro, digite um mês válido.\n"); 
    ...
    while(1) {
        ...
        if(num == 0) return 1;
            ...
            if(bissexto == 0 && d->dia == 29)printf("Ano de data inválido, pois o ano não é bissexto, digite um ano válido.\n");
            else {
...
int pick_email(char * s) {
    ...
    while(1) {
        ...
        scanf("%s", s);
            ...
            if(s[0] == '@')printf("Email inválido, por favor digite novamente.\n");
            ...
            if(arroba != 1)printf("Email inválido, por favor digite novamente.\n");
            ...
            for(i=a+1; s[i] != '\0'; i++){ //começa da posição em que achamos o @ até o final procurando pelo final do domínio;
                ...
                if(s[i] == '.'){  // contabilizando os pontos;
                    ...
                    if(s[i+1] == '.' || s[i+1] == '\0' || s[a+1] == '.'){
                        printf("Email inválido, por favor digite novamente.\n");
                        aux++;
                    }
                }
            }
            ...
            if(aux > 0)printf("Email inválido, por favor digite novamente.\n");
